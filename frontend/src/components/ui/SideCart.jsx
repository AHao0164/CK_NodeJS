      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white dark:bg-gray-900 shadow-2xl z-[101] transform transition-transform duration-300 ease-in-out ${
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-3">
            <h2 className="text-5xl font-normal font-bitcount">Giỏ hàng</h2>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {!user ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-500 dark:text-gray-400 mb-2">{VI.cart.emptyCart}</h3>
              <p className="text-gray-400 dark:text-gray-500">Thêm sản phẩm để bắt đầu</p>
            </div>
          ) : (
              ...
              {(cart.items || []).map((item) => (
                <div key={item.id} className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                  ...
                  <div className="flex-1">
                    <h3 className="font-semibold text-gray-900 dark:text-white mb-1">Sản phẩm {item.product_id}</h3>
