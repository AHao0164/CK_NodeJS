...
export async function listNewProducts({ limit = 8 } = {}) {
  const { data } = await publicApi.get('/catalog/products-new', { params: { limit } })
  return Array.isArray(data?.items) ? data.items : []
}

export async function listBestsellerProducts({ limit = 8 } = {}) {
  const { data } = await publicApi.get('/catalog/products-bestsellers', { params: { limit } })
  return Array.isArray(data?.items) ? data.items : []
}
