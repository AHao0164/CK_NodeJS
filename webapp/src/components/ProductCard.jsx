    ...
    return (
        <div className="card group flex flex-col overflow-hidden">
            ...
            <div className="flex flex-1 flex-col gap-2 p-4">
                <Link to={`/product/${product.id}`} className="line-clamp-1 text-sm font-semibold text-slate-900 hover:underline">
                    {product.name}
                </Link>
                ...
                <div className="mt-auto flex items-center justify-between">
                    ...
                    <button onClick={() => onAdd?.(product)} className="btn btn-primary rounded-xl">Thêm</button>
                </div>
            </div>
        </div>
    )
