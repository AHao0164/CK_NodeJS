...
export default function Cart() {
  ...
  useEffect(() => { (async () => {
    ...
    setCart(c); setCatalog(Array.isArray(p?.items) ? p.items : [])
  })() }, [api])
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-8">
      <h2 className="text-2xl font-semibold tracking-tight">Giỏ hàng</h2>
            ...
            <CardBody>
              {cart.items.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <div className="mt-3 text-sm text-slate-600 dark:text-slate-300">Giỏ hàng trống. Khám phá sản phẩm để bắt đầu mua sắm!</div>
                </div>
              ) : (
                      ...
                      <div className="flex items-center justify-between gap-3 py-3" key={it.id}>
                        <div className="flex min-w-0 items-center gap-3 pr-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="mt-0.5 text-xs text-slate-500">Số lượng x{it.quantity}</div>
                          </div>
                        </div>
          ...
          <Card>
            <CardBody>
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between font-semibold"><span>Tổng thanh toán</span><span>{(total/100).toLocaleString()} ₫</span></div>
              </div>
              <Link to="/checkout"><Button className="mt-4 w-full" disabled={cart.items.length===0}>Thanh toán</Button></Link>
              <Link to="/" className="mt-2 inline-flex w-full justify-center text-sm text-brand-600 hover:underline">Tiếp tục mua sắm</Link>
            </CardBody>
          </Card>
        </div>
      </div>
